...
int main(void)
{
    struct timeval time;
    const int N = 4*4;
    thrust::device_vector<uint64_t> V1(N);
    thrust::device_vector<uint64_t> V2(N);
    thrust::device_vector<uint64_t> V3(N);
    thrust::sequence(V1.begin(), V1.end(), 1);
    thrust::fill(V2.begin(), V2.end(), 1);
    gettimeofday(&time, NULL);
    int begin = time.tv_usec;
    gpu::matrixMultiplication(&V1, &V2, &V3, false, false, 4, 4, 4, 4);
    gettimeofday(&time, NULL);
    int end = time.tv_usec;
    std::cout<< end-begin <<std::endl;
    // print V3
    std::vector<uint64_t>stdv1(N,1);
    std::vector<uint64_t>stdv2(N,75);
    gettimeofday(&time, NULL);
    begin = time.tv_usec;
    for(int i=0; i<N; i++){
        stdv1[i] = stdv1[i] * stdv2[i];
    }
    gettimeofday(&time, NULL);
    end = time.tv_usec;
    std::cout<< end-begin <<std::endl;
